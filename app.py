...
def base64_to_pil(b64_str):
    """Convert a base64 data-URI or plain base64 string to a PIL Image."""
...
def generate_excel():
      ...
      "items": [
        {
          ...
          "qty":    50,
          "remark": "",
          "img":    "data:image/jpeg;base64,..."  // or "" if no photo
        }
      ]
    }
    ...
    # ── Header row ─────────────────────────────────────────────────────────────
    headers = ["Photo", "CTN NO", "ITEM NO", "DESCRIPTION", "CATEGORIE",
               "PRICE", "PCS/CTN", "CTN", "QTY", "AMOUNT", "REMARK"]
    col_widths = [14,     10,       12,       28,             22,
                  10,     10,       6,        8,    12,        35]
    ...
    # ── Data rows ──────────────────────────────────────────────────────────────
    IMG_ROW_HEIGHT = 62   # px → points (approx)
    IMG_SIZE       = (60, 60)
    total_amount   = 0.0
    ...
    for row_offset, item in enumerate(items):
        ...
        # Cols B–K (text data — col A reserved for image)
        values = [
            item.get("group",  ""),
            item.get("ref",    ""),
            item.get("desc",   ""),
        ...
        if img_b64 and len(img_b64) > 100:
            try:
                ...
                # If image fails, just leave cell empty
                cell = ws.cell(row=r, column=1, value="[img err]")
                cell.fill   = hex_fill(bg)
                cell.border = thin_border()
        else:
